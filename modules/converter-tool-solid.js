...
// GANTI template header yang ada DENGAN:
...
function pkmShowNotification(title, text, options) {
    console.log(\`%c📢 \${title}: \${text}\`, 'background: #3498db; color: white; padding: 5px;');
...
`;
    ...
    convert(code) {
        ...
        try {
            ...
            console.log('✅ Metadata parsed:', metadata.originalName);
    ...
    addWarning(message) {
        ...
        console.warn('⚠️ Converter Warning:', message);
    },
...
function setupConverterUI() {
    ...
    ">
        <div>
            <div style="font-weight: bold; font-size: 16px;">🔄 PKM Converter</div>
...
function processConversion() {
    ...
    if (!code) {
        updateStatus('❌ Masukkan kode terlebih dahulu', 'error');
        return;
    }
    
    updateStatus('🔄 Memproses konversi...', 'processing');
    
    // Perform conversion
    const result = PKMConverter.convert(code);
    
    if (!result) {
        updateStatus('❌ Konversi gagal', 'error');
    ...
    updateStatus(`✅ ${result.metadata.originalName} berhasil dikonversi!`, 'success');
}
...
function showMessages(errors, warnings) {
    ...
    if (errors.length > 0) {
        ...
        html += '<strong>⚠️ Errors:</strong><br>';
        errors.forEach(err => {
            html += `• ${err}<br>`;
        });
        html += '</div>';
...
function downloadFile(content, filename, type) {
    ...
    updateStatus(`✅ ${filename} berhasil di-download`, 'success');
}
